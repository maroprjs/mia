...
void GsmModem::begin(){
  ...
  //_buffer.reserve(255); //reserve memory to prevent intern fragmention
	...
	delay(500);
	if(heartBeat()){
		PRINTLN("Modem is Alive");
		//sleep();
		delay(1000);/*
		sendCheckReply(F("AT+IPR=4800"), _ok_reply);// Fix UART speed to 9600 bps
		delay(1000);
		sendCheckReply(F("AT+CREG=0"), _ok_reply); //disable reporting URC of losing 2G coverage by +CREG=0
		delay(1000);
		sendCheckReply(F("AT&W"), _ok_reply); //save config
		delay(3000);*/
		sendCheckReply(F("AT+CMGF=1"), _ok_reply);
		delay(1000);
		PRINTLN("display automatically SMS when arrives: ");
		sendCheckReply(F("AT+CNMI=1,2,0,0,0"), _ok_reply);
		delay(1000);
	}else{
		PRINTLN("Modem doesnt answer AT");
	};
};
...
bool GsmModem::smsArrived(void){
	readline(2000);
	//PRINTLN(strstr(_replybuffer, "CMT:") );
	if ((strstr(_replybuffer, "CMT:") != NULL) && (strstr(_replybuffer, "MMS") == NULL)){
		//PRINT(" SMS arrived: ");
...
bool GsmModem::delAllSms(SMS *msg){
	/*orig:
  ...
  if (_buffer.indexOf("OK")!=-1) {return true;}else {return false;}
	...
	return "to be done!";*/
	msg->msg = "";
	sendCheckReply(F("AT+CMGD=4"), _ok_reply);
...
boolean GsmModem::getSMSSender(uint8_t i, char *sender, int senderlen) {
  ...
  boolean result = parseReplyQuoted(F("+CMT: "), sender, senderlen, ',', 0);
  return result;
}
